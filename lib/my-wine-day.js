...
const AREAS = new Set(["any", "petoskey", "harbor-alanson", "boyne-walloon", "charlevoix"]);
const PACES = new Set(["leisurely", "standard", "efficient"]);
const BEVERAGES = new Set(["wine", "cider", "beer", "spirits", "mead"]);
const POI_KINDS = new Set(["beach", "hike", "scenic", "lighthouse", "town", "paddle", "market", "food", "history"]);
...
export function winePlanLabel(raw, options = {}) {
  ...
  const areaLabel = plan.area === "petoskey"
    ? "Petoskey"
    : plan.area === "harbor-alanson"
      ? "Harbor Springs and Alanson"
      : plan.area === "boyne-walloon"
        ? "Walloon Lake and Boyne"
        : plan.area === "charlevoix"
          ? "Charlevoix"
          : "Wine country";
