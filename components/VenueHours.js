...
export const HOURS_VERIFIED = "September 18, 2026";
...
export function condenseWeek(hours) {
  ...
  return runs
    .map((r) => {
      const label = r.start === r.end ? DAY_ABBR[r.start] : `${DAY_ABBR[r.start]}-${DAY_ABBR[r.end]}`;
      return `${label} ${fmtTime(r.open)}-${fmtTime(r.close)}`;
    })
    .join(", ");
}

export default function VenueHours({ title, areas, towns, categories, trailOnly }) {
  let list = venues;
  ...
  if (trailOnly) list = list.filter((v) => v.officialTrail);
        ...
        {list.map((v) => (
          <li key={v.id}>
            ...
            {v.needsHours || !v.hours || !Object.keys(v.hours).length ? (
              <>call ahead for current hours{v.phone ? ` at ${v.phone}` : ""}</>
            ) : (
