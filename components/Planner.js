...
export default function Planner({
  preset = {},
  embedded = false,
  title = "Petoskey Wine Region Map",
  description = "Map the wineries of the Petoskey Wine Region from Walloon Lake to Harbor Springs and Charlevoix, pick the stops you want, then route the day on real roads against posted hours.",
}) {
  useEffect(() => {
    ...
    const AREA_LABEL = { "petoskey":"Petoskey", "harbor-alanson":"Harbor Springs and Alanson", "boyne-walloon":"Walloon Lake and Boyne", "charlevoix":"Charlevoix area" };
    const AREA_ORDER = ["petoskey","boyne-walloon","harbor-alanson","charlevoix"];
    ...
    const state = {
      ...
      area:["any","petoskey","harbor-alanson","boyne-walloon","charlevoix"].includes(preset.area) ? preset.area : "any",
      origin:ORIGINS[preset.origin] ? preset.origin : "Petoskey",
    ...
    const STARTERS = [
      { id:"walloon-loop", title:"The Walloon Lake loop", meta:"3 wineries · Walloon Lake and Petoskey",
        blurb:"The tight southern cluster: Walloon Lake Winery, then Rudbeckia and Boyne Valley, with the village of Walloon Lake to start or finish in.",
        origin:"Walloon Lake", beverages:["wine"], poiKinds:["town"],
        stops:["walloon-lake-winery","rudbeckia","boyne-valley-vineyards","walloon-lake-village"] },
      { id:"petoskey-day", title:"A Petoskey wine day", meta:"3 wineries and a stone beach · Petoskey",
        blurb:"Atkins Road and US-131 south of town, finished on the bay at Petoskey State Park where the stone hunting is.",
        origin:"Petoskey", beverages:["wine"], poiKinds:["beach"],
        stops:["petoskey-farms","maple-moon","mackinaw-trail","petoskey-state-park"] },
      { id:"shoreline-run", title:"Bay shore to Charlevoix", meta:"3 wineries · Bay Shore and Charlevoix",
        blurb:"The shoreline run west from Petoskey: Spare Key on Upper Bay Shore Road, Blu Dot outside Charlevoix, then the pier light and downtown.",
        origin:"Petoskey", beverages:["wine"], poiKinds:["lighthouse"],
        stops:["spare-key","blu-dot","1918-cellars","charlevoix-south-pier"] },
    ];
    ...
    function planSummaryText(){
      ...
      const lines=[`Our Petoskey wine day · ${dayName(state.date)} · start ${pretty(toMin(state.start))} from ${state.origin}`];
    ...
    async function shareExactWinePlan(){
      ...
      if(navigator.share){
        try{
          await navigator.share({title:"Our Petoskey wine day",text,url});
          method="native";
        }catch(err){
    ...
    function initMap(){
      const initialView = state.area === "petoskey"
        ? { center:[45.36,-84.94], zoom:11 }
        : state.area === "boyne-walloon"
          ? { center:[45.25,-84.96], zoom:11 }
          : state.area === "harbor-alanson"
            ? { center:[45.44,-84.90], zoom:10 }
      ...
      L.marker([45.0,-84.66],{interactive:false,icon:L.divIcon({className:"par-label",html:"45th parallel",iconSize:[80,14]})}).addTo(map);
    ...
    function buildChips(){
      ...
      [["any","All areas"],["petoskey","Petoskey"],["boyne-walloon","Walloon and Boyne"],["harbor-alanson","Harbor and Alanson"],["charlevoix","Charlevoix"]].forEach(([k,lab])=>{ const el=document.createElement("button"); el.className="chip"+(state.area===k?" chip-on":""); el.dataset.area=k; el.textContent=lab; ac.appendChild(el); });
